...
/**
 * Default context strategy used when no context.yaml exists
 *
 * This ensures backward compatibility and provides sensible defaults:
 * 1. System prompt (agent instructions)
 * 2. DELTA.md (workspace guide, auto-skip if missing)
 * 3. Journal source with unlimited history (max_iterations: undefined)
 *
 * Design rationale: ALWAYS include journal source. Not configuring max_iterations
 * means unlimited conversation history (rebuild all). Users can add max_iterations
 * to limit token usage when needed.
 */
export const DEFAULT_MANIFEST: ContextManifest = {
  sources: [
    ...
    },
    {
      ...
      // max_iterations: undefined means unlimited (rebuild all conversation)
    },
  ],
};
