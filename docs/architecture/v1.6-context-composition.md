...
const DEFAULT_MANIFEST: ContextManifest = {
  sources: [
    ...
    // No journal source by default!
  ]
};
```

**Engine fallback**: When no `journal` source is present in context.yaml (or when using default manifest), the engine falls back to v1.5 behavior: rebuild **all** conversation history from journal.jsonl.

This means:
1. Every agent gets **workspace guide support** (`DELTA.md`) for free
2. **Zero-config = full conversation** (no token limits applied)
3. Creating `context.yaml` allows explicit control over conversation history
4. **User responsibility**: Users must actively decide if/when to limit context
...
**Engine fallback**: If no `journal` source is present in context.yaml, the engine automatically rebuilds **all** conversation from journal.jsonl (v1.5 behavior).
